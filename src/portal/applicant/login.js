...
export default function LoginApplicant(){
    ...
    const handleSubmit = async ()=>{
        try{
            const {data:response} = await axios.post(`${SERVER_URL}/candidate/registered/login`,{
                email,password
            });
            if(!response){
            ...
            return navigate("/portal/applicant")
        }catch(err){
                ...
                <Container className="d-flex vh-100 justify-content-center align-items-center">
                        <Card style={{maxWidth:"50vw"}}>
                            <Card.Header className="text-center d-flex flex-column align-items-center">
                                ...
                                <span className="fw-bold text-warning h3">Login Applicant</span>
                            </Card.Header>
                            <Card.Body>
                                <Form>
                                <FloatingLabel label="Email" controlId="email" className="mb-3">
                                    <Form.Control type="email" onChange={(e)=>setEmail(e.target.value)} className="rounded-0" placeholder="email"/>
                                </FloatingLabel>
